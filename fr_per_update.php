...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if (empty($password)) {
		...
		$valid = true;								//Blank password means password is unchanged
		$passwordChanged = false;
	}
	else{
	...
	if ($passwordChanged) {
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$q = $pdo->prepare($sql);
			$q->execute(array(htmlspecialchars($fname, ENT_QUOTES), htmlspecialchars($lname, ENT_QUOTES), htmlspecialchars($email, ENT_QUOTES), htmlspecialchars($mobile, ENT_QUOTES), htmlspecialchars($passwordhash, ENT_QUOTES), htmlspecialchars($title, ENT_QUOTES), $fileName,$fileSize,$fileType,$content, htmlspecialchars($address, ENT_QUOTES), htmlspecialchars($city, ENT_QUOTES), htmlspecialchars($state, ENT_QUOTES), htmlspecialchars($zip, ENT_QUOTES), $id));
			Database::disconnect();
			...
			$q = $pdo->prepare($sql);
			$q->execute(array(htmlspecialchars($fname, ENT_QUOTES), htmlspecialchars($lname, ENT_QUOTES), htmlspecialchars($email, ENT_QUOTES), htmlspecialchars($mobile, ENT_QUOTES), htmlspecialchars($passwordhash, ENT_QUOTES), htmlspecialchars($title, ENT_QUOTES), htmlspecialchars($address, ENT_QUOTES), htmlspecialchars($city, ENT_QUOTES), htmlspecialchars($state, ENT_QUOTES), htmlspecialchars($zip, ENT_QUOTES), $id));
			Database::disconnect();
			header("Location: fr_persons.php");
		}
		
	}
	else{	//if password is not changed, then don't update passowrd
		if($fileSize > 0) { // if file was updated, update all fields
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, email = ?, mobile = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ?, address = ?, city = ?, state = ?, zip = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array(htmlspecialchars($fname, ENT_QUOTES), htmlspecialchars($lname, ENT_QUOTES), htmlspecialchars($email, ENT_QUOTES), htmlspecialchars($mobile, ENT_QUOTES), htmlspecialchars($title, ENT_QUOTES), $fileName,$fileSize,$fileType,$content,htmlspecialchars($address, ENT_QUOTES), htmlspecialchars($city, ENT_QUOTES), htmlspecialchars($state, ENT_QUOTES), htmlspecialchars($zip, ENT_QUOTES), $id));
			Database::disconnect();
			header("Location: fr_persons.php");
		}
		else { // otherwise, update all fields EXCEPT file fields
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, email = ?, mobile = ?, title = ?, address = ?, city = ?, state = ?, zip = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array(htmlspecialchars($fname, ENT_QUOTES), htmlspecialchars($lname, ENT_QUOTES), htmlspecialchars($email, ENT_QUOTES), htmlspecialchars($mobile, ENT_QUOTES), htmlspecialchars($title, ENT_QUOTES), htmlspecialchars($address, ENT_QUOTES), htmlspecialchars($city, ENT_QUOTES), htmlspecialchars($state, ENT_QUOTES), htmlspecialchars($zip, ENT_QUOTES), $id));
			Database::disconnect();
			header("Location: fr_persons.php");
		}
	}
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="fr_per_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input name="address" type="text"  placeholder="Mailing Address" value="<?php echo !empty($address)?$address:'';?>">
					...
					<div class="controls">
						<input name="state" type="text"  placeholder="State (Abbr)" value="<?php echo !empty($state)?$state:'';?>">
